    ...
    async function renderLaporan() {
      ...
      el.innerHTML = '<div style="text-align:center;padding:32px;color:var(--txt3);font-size:12px">Memuat laporan...</div>';
      
      let allTodayOrders = HISTORY;
      try {
        const today = new Date().toISOString().split('T')[0];
        const res = await API.orders.getAll({ limit: 1000, dateFrom: today, dateTo: today });
        if (res && res.orders) {
          allTodayOrders = res.orders.map(o => {
            return {
              id: o.id,
              no: o.order_number,
              ...
              items: (o.items || []).map(it => ({ n: it.name, qty: it.quantity, p: it.unit_price }))
            };
          });
        }
      } catch (e) { console.error('Failed to fetch full history for laporan', e); }

      var PMC = { 'Tunai': '#22C55E', 'QRIS': '#3B82F6', 'BCA': '#1E40AF', 'Debit': '#A855F7', 'GoFood': '#E3175B', 'GrabFood': '#00B14F', 'ShopeeFood': '#EE4D2D', 'Transfer': '#06B6D4' };
      var TYC = { 'dine': '#E4540C', 'take': '#3B82F6', 'gofood': '#E3175B', 'grabfood': '#00B14F', 'shopee': '#EE4D2D', 'GoFood': '#E3175B', 'GrabFood': '#00B14F', 'ShopeeFood': '#EE4D2D', 'dine-in': '#E4540C', 'takeaway': '#3B82F6', 'shopeefood': '#EE4D2D' };
      var TYL = { 'dine': 'Dine In', 'take': 'Take Away', 'gofood': 'GoFood', 'grabfood': 'GrabFood', 'shopee': 'ShopeeFood', 'GoFood': 'GoFood', 'GrabFood': 'GrabFood', 'ShopeeFood': 'ShopeeFood', 'dine-in': 'Dine In', 'takeaway': 'Take Away', 'shopeefood': 'ShopeeFood' };
      var done = allTodayOrders.filter(function (h) { return h.status === 'done'; });
      var voided = allTodayOrders.filter(function (h) { return h.status === 'voided' || h.status === 'cancelled'; });
      ...
      h += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><div class="lap-bo-note" style="margin-bottom:0"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>' + ds + ' — Laporan hari ini. Riwayat lengkap tersedia di Backoffice.</div><button class="btn" style="padding:6px 12px;font-size:12px" onclick="renderLaporan()"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:6px"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>Refresh</button></div>';
