...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await run('UPDATE modifier_groups SET status = ?, updated_at = ? WHERE id = ?', ['inactive', new Date().toISOString(), id]);
...
router.delete('/options/:optionId', async (req, res) => {
  try {
    ...
    await run('UPDATE modifier_options SET status = ? WHERE id = ?', ['inactive', optionId]);
...
router.post('/:id/assign-product', async (req, res) => {
  try {
    const { id } = req.params;
    ...
    await run('UPDATE products SET has_modifiers = 1, updated_at = ? WHERE id = ?', [new Date().toISOString(), productId]);
...
router.delete('/:id/unassign-product/:productId', async (req, res) => {
  try {
